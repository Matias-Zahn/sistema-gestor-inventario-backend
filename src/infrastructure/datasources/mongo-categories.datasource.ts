...
export class MongoCategoriesDataSource implements CategoryDataSource {
  ...
  async getOneCategory(term: string): Promise<CategoryEntity> {
    ...
    if (!existCategory)
      throw CustomError.notFound(
        `Categoria no encontrada por el termino: ${term}`,
      );
  ...
  async createCategory(
    ...
    if (!newCategory)
      throw CustomError.internalServerError(
        "Algo ocurrio con la creacion de la categoria",
      );
  ...
  async updateCategory(
    ...
    const updated = await CategoryModel.findByIdAndUpdate(
      category.id,
      updateCategoryDTO.values,
      { new: true },
    );

    if (!updated)
      throw CustomError.internalServerError(
        "ALgo ocurrio cuando se intentaba actualizar la categoria",
      );
